...
public class LinkedList<E extends Comparable<E>>
    ...
    public boolean addAll(int index, LinkedList<? extends E> c){
        ...
        for(Node<? extends E> d2 = c.first; d2 != null; d2 = d2.next){
        
         
        
        Node<E> thisone = first.findNode(index); // esamas
        Node<E> adding = new Node(d2, thisone.next); // pridedamas
        if(thisone == first) {
            first = adding;
            first.next = thisone;
        }
        else{
            Node<E> previous = first.findNode(index-1); // esamas
    ...
    public boolean add(int k, E e) {
        ...
        Node<E> thisone = first.findNode(k); // esamas
        Node<E> adding = new Node(e, thisone.next); // pridedamas
        if(thisone == first) {
            first = adding;
            first.next = thisone;
        }
        else{
            Node<E> previous = first.findNode(k-1); // esamas
            previous.next = adding;
            adding.next = thisone;
        }   
    ...
    public E remove(int k) {
        ...
        if(thisone == first) {
            first = thisone.next;
            first.next = thisone.next.next;
        }
        else{
            Node<E> previous = first.findNode(k-1); // esamas
            previous.next = thisone.next;
        }   
    ...
    public E removeFirst(){
      ...
      public boolean containsAll(LinkedList<E> c){
        int allequals = 0;
        for(Node<E> d1 = first; d1 != null; d1 = d1.next){ // ciklas per sąrašo objektus
            for(Node<E> d2 = c.first; d2 != null; d2 = d2.next){
                if(d1.element.compareTo(d2.element) == 1){
                    allequals++;
                }
            }
        }
        if(allequals == c.size) return true;
        return false;
    }
    
    public int indexOf(Object o){
        for(int i = 0; i < size; i++){
            if(first.findNode(i).element.equals(o)) return i;
        }
        return -1;
    }
